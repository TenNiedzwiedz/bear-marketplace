...
- **Design system.** Reusable UI lives in `resources/js/components/` (`AppButton`, `SellerBadge`, `AppTag`, `SearchField`, `ListingCard`, `BearSeal`, `SectionBlock`, `EyebrowLabel`). Compose production views from these rather than restyling from scratch. Design tokens are defined in `resources/css/app.css`: a fixed brand palette in `@theme` (`--color-pine/paper/honey/moss/cranberry/taupe`) plus theme-aware semantic tokens (`--bg`, `--surface`, `--text`, `--accent`, `--seller-firma`, …) that swap under `prefers-color-scheme` and an explicit `data-theme` on `<html>`. Components read the semantic tokens via scoped styles — never hard-code hex. The live reference is the `/design-cheatsheet` page (`resources/js/pages/DesignCheatsheet.vue`), which documents palette, type scale, and every component with usage snippets.
